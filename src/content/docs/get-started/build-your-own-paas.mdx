...
This guide uses a specific set of add-ons to demonstrate the process. You can choose to install any add-ons you like, according to your needs, either through the Edka dashboard, GitOps or directly through your Kubernetes cluster API.
...
- integration with an external container registry to enable automatic image pull/push for your workloads
...
## Install Add-ons
In your cluster overview, navigate to **Settings** > **Add-ons**
...
*Note: this example uses [Doppler](https://www.doppler.com/) as the external secrets provider, but you can use any other supported provider.*
...
In this example we have connected [edkadigital/clusterone](https://github.com/edkadigital/clusterone) repository to the cluster, which provides a blueprint for the cluster and add-ons configuration. You can inspect the GitOps reconciliation status in the cluster, using `kubectl` command line tool or Lens IDE.
